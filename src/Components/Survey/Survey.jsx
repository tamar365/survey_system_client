...
function Survey() {
  ...
  const [arrayOfObjectOfQuestionsToDB, setArrayOfObjectOfQuestionsToDB] =
    useState([]);
  ...
  const addQuestionToContainer = () => {
    ...
    setArrayOfCreatedQuestionsUI([
      ...arrayOfCreatedQuestionsUI,
      numberOfQuestion,
    ]);
  };

  const saveOneQuestionFunc = () => {
    setArrayOfObjectOfQuestionsToDB([
      ...arrayOfObjectOfQuestionsToDB,
      {
    ...
    try {
      ...
      if (status === 200) {
        ...
        data.questions.filter((item) =>
          item.optionOfQuestion === "שאלה פתוחה"
            ? setOpenQuestionOption(true)
            : item.optionOfQuestion === "שאלת דירוג"
            ? setScaleQuestionOption(true)
            : null
        );
      } else {
  ...
  if (idOfSurvey && savedDetailsOfSurvey) {
    navigate(`/ExampleOfCreatedSurvey/${idOfSurvey}`, {
      state: {
          ...
          <div className="inputsContainer">
            <div className="addQuestionContainer">
              <button
                ...
                onClick={addQuestionToContainer}
              >
            ...
            <div className="questionsContainer">
              {arrayOfCreatedQuestionsUI.length
                ? arrayOfCreatedQuestionsUI.map((question) => (
                    <QuestionBox
                      id={question.numberOfQuestion}
                      setOptionOfQuestion={setOptionOfQuestion}
                      setTheWrittenQuestion={setTheWrittenQuestion}
                      saveOneQuestionFunc={saveOneQuestionFunc}
                    />
                  ))
                : undefined}
            </div>
            <button className="saveButton" onClick={saveTheQuestions}>
              שמור סקר
            </button>
          </div>
        </div>
        {openPopUp && (
          <PopUpForSurvey message={message} setOpenPopUp={setOpenPopUp} />
        )}
      </div>
